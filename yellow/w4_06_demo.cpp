...
void PrintStats(vector<Person> persons){
  ...
  // Median age
  auto it = partition(persons.begin(), persons.end(), [](const Person& p){return true;});
  cout << "Median age = " << ComputeMedianAge(persons.begin(), it) << endl;

  // Median age for females
  it = partition(persons.begin(), persons.end(), [](const Person& p){return p.gender == Gender::FEMALE;});
